...
public class PlayerEventsPW {
    ...
    // Other
    public static void setWGetMount() {
        if (PlayerEventsConfig.getConfig().getBoolean("Block-Mount.Enable") && !PlayerEventsConfig.getConfig().getBoolean("Block-Mount.World.All_World")) {
            for (String world: PlayerEventsConfig.getConfig().getStringList("Block-Mount.World.Worlds")) {
                if (Bukkit.getWorld(world) == null) {
                    System.out.println("| Invalid world in Events/PlayerEvents.yml, Block-Mount.World: " + world);
                } else {
                	worlds_block_mount.add(world);
                }
            }
        }
    }

    public static List < String > getWM() {
        return worlds_block_mount;
    }
